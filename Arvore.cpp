...
void Arvore::colocarSubArvoreEmVetor(no *p, Palavra palavra,Palavra *vet,int *posicao)
{
    if (p == NULL)
    {
        ...
        if (palavra > p->dado){
            if (palavra.semelhante(p->dado))
            {
                ...
                *(vet+*posicao)=p->dado;
                *posicao=*posicao+1;
                colocarSubArvoreEmVetor(p->esq, palavra,vet,posicao);
                colocarSubArvoreEmVetor(p->dir, palavra,vet,posicao);
            }
            else
            colocarSubArvoreEmVetor(p->dir, palavra,vet,posicao);
        }else{
